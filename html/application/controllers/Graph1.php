<?
    class Graph1 extends CI_Controller {       // graph클래스 선언
        function __construct()                           // 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();                     // 데이터베이스 연결
            $this->load->model("graph_m1");    // 모델 graph_m1 연결
    ...
    // $page_segment = 6;
			...
			$data["rowcount"] = $this->graph_m1->rowcount($text1,$text2);
			$data["text1"]=$text1;  
			$data["text2"]=$text2;
            $data["list"]=$this->graph_m1->getlist();   // 해당페이지 자료읽기
			//$data["list1"]=$this->graph_m1->getlist1($text1,$text2);   // 해당페이지 자료읽기
			//$data["list2"]=$this->graph_m1->getlist2($text1,$text2);   // 해당페이지 자료읽기
 
 

            $this->load->view("admin_header");                    // 상단출력(메뉴)
            $this->load->view("graph1_list",$data);           // graph_list에 자료전달
