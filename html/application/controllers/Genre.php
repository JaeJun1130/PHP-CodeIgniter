<?
    class Genre extends CI_Controller {       // Member클래스 선언
        function __construct()                           // 클래스생성할 때 초기설정
        {
            parent::__construct();
            $this->load->database();                     // 데이터베이스 연결
            $this->load->model("genre_m");    // 모델 Member_m 연결
		...
		public function view()
		{
			...
			$this->load->view("admin_header");                    // 상단출력(메뉴)
            $this->load->view("genre_view",$data);           // genre_list에 자료전달
            $this->load->view("admin_footer");                      // 하단 출력 
		}
		public function del()
		{
			...
			redirect("/~team17/genre/lists" . $text1 . $page);            // 목록화면으로 돌아가기
		}
		public function add()
		{
			...
			if (!$_POST ) // 목록화면의 추가버튼 클릭한 경우
			{
			 ...
			 else              // 입력화면의 저장버튼 클릭한 경우
			 {
				...
				redirect("/~team17/genre/lists/". $text1 .$page);    //   목록화면으로 이동.
			 }
		 }
